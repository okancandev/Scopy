...
{
    class TreeViewExample : TreeView
    {
        public TreeViewExample(TreeViewState state, MultiColumnHeader header)
            : base(state, header)
        {
        ...
        void CellGUI(Rect cellRect, ServiceCustomTreeViewItem item, int column, ref RowGUIArgs args)
        {
            switch (column)
            {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    if (item.Service is IScopyEditorCustomGUI customGUIService)
                    {
                        EditorGUI.LabelField(cellRect, customGUIService.OnScopyEditorExtraField());
                    }
                    break;
            }
        }
    ...
    sealed class ScopeCustomTreeViewItem : TreeViewItem
    {
        ...
        public ScopeCustomTreeViewItem(int id, object owner)
        {
            ...
            else if (Owner is Scene sceneOwner)
            {
                ...
                icon = EditorGUIUtility.FindTexture("SceneAsset On Icon");
            }
